...
**Versión actual: v2 COMPLETA (Fases A–E) + extras de producto. Señal real BrainFlow/dataset/LSL (LslSource), decoder motor imagery BCI IV 2a (CV 63.4%), gateway FastAPI (JWT/scopes/revocación), hardening (cifrado HKDF/AES-GCM por app con rotación y anti-replay podado, log firmado Ed25519 con verify_audit.py y append atómico, anomalía sobre telemetría real por ventana deslizante, TLS dev, 8 ataques bloqueados y auditados), dashboard cliente del servicio + despliegue (Dockerfile/compose/DEPLOY.md). Extras: SDK `neurogate-client` (sdk/) e informe de cumplimiento firmado (compliance_report.py). Hardening de seguridad post-revisión aplicado (secretos no-placeholder, lock del log, verify_token 401, auditoría completa). 118 tests verdes. Venv Python 3.12. v1 COMPLETA (Pasos 1–11). Demo pública: https://neurogate.streamlit.app · Blog: https://earnlydriver-code.github.io/neurogate. Próximo posible: KMS, multi-tenant, pentest externo, validación con hardware físico, go-to-market (ver reports/PLAN-VENTA.md, local).**
